...
class MessagingService(object):
    ...
    def get_active_conversations(self, sender, recipient):
        ...
        active_conversations = Message.objects.filter((Q(sender=sender) & Q(recipient=recipient)) | (
            Q(sender=recipient) & Q(recipient=sender))).order_by('sent_at')

        print('active conversations', active_conversations)
    ...
    def get_conversations(self, user):
        ...
        #     .annotate(max_date=Max('sent_at')).filter(sent_at=F('max_date'))
            ...
            if conversation['recipient'] != user.pk:
                ...
                if conversation['recipient'] not in contacts:
                    ...
                    chatroom = ChatRoom.objects.filter(
                        ...
                        (Q(sender=recp) & Q(recipient=sndr))).first()
